...
serve(async(req:Request)=>{
  ...
  if(type!=='mobile') return jsonResponse({error:'Meetup verification is mobile-only. Scan the transfer-to-mobile QR code to continue.',code:'MOBILE_REQUIRED'},403,{},req);
  ...
  if(!transactionId||!role||lat===null||lng===null) return jsonResponse({error:'Missing arrival fields'},400,{},req);
  const userClient=createUserClient(token);
  const {data,error}=await admin.rpc('record_safe_arrival_v2',{p_transaction_id:transactionId,p_role:role,p_lat:lat,p_lng:lng,p_user_id:user.id,p_device_type:type});
