...
async function saveChallenge(userId:string,type:"reg"|"auth",challenge:string){const key=`challenge:${userId}:${type}`;cache.set(key,challenge);await admin.from("user_security").upsert({user_id:userId,device_hash:key,device_label:`__challenge__${challenge}`,flag_type:"webauthn_challenge",created_at:new Date().toISOString()},{onConflict:"user_id,device_hash"});}
async function loadChallenge(userId:string,type:"reg"|"auth"){const key=`challenge:${userId}:${type}`;if(cache.has(key))return cache.get(key)!;const {data}=await admin.from("user_security").select("device_label,created_at").eq("user_id",userId).eq("device_hash",key).maybeSingle();if(!data?.device_label)return null;if(Date.now()-new Date(data.created_at).getTime()>CHALLENGE_TTL_MS){await clearChallenge(userId,type);return null;}const challenge=data.device_label.replace("__challenge__","");cache.set(key,challenge);return challenge;}
async function clearChallenge(userId:string,type:"reg"|"auth"){const key=`challenge:${userId}:${type}`;cache.delete(key);await admin.from("user_security").delete().eq("user_id",userId).eq("device_hash",key);}
...
serve(async(req:Request)=>{
  ...
  if(action==="verify_registration"){
    ...
    const {error}=await admin.from("passkey_credentials").insert({user_id:userId,credential_id:credentialID,public_key:Buffer.from(credentialPublicKey).toString("base64url"),sign_count:counter,transports:[],device_label:deviceLabel||"My Passkey",backed_up:credentialBackedUp});
