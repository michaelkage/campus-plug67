...
serve(async (req:Request) => {
  ...
  try {
    ...
    if(!limit.allowed) return bad("Rate limit exceeded",429);
  } catch { return bad("Rate limit service unavailable",503); }
  ...
  if(action==="update_beacon"){
    const {error:upsertErr}=await admin.from("ticker_events").upsert({user_id:user.id,event_type:"beacon_current",latitude,longitude,transaction_id:transaction_id??null,metadata:{beacon_type,transaction_id},created_at:now},{onConflict:"user_id,event_type"});
    if(upsertErr) return bad("Failed to update beacon: "+upsertErr.message,500);
    const {error:historyErr}=await admin.from("ticker_events").insert({user_id:user.id,event_type:"beacon_update",latitude,longitude,transaction_id:transaction_id??null,metadata:{beacon_type,transaction_id},created_at:now});
