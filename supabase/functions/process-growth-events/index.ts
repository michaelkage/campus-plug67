...
const admin = url && privilegedKey
  ? createClient(url, privilegedKey, { auth: { persistSession: false, autoRefreshToken: false } })
  : null;
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (action === "referral_signup") {
      ...
      if (referrerError) return bad(req, referrerError.message, 500);
      ...
      if (existingError) return bad(req, existingError.message, 500);
      ...
      if (profileError) return bad(req, profileError.message, 500);
      ...
      if (referralError) return bad(req, referralError.message, 500);
    ...
    if (action === "gig_response") {
      ...
      if (gigError) return bad(req, gigError.message, 500);
    ...
    if (action === "log_view") {
      ...
      if (viewError) return bad(req, viewError.message, 500);
    ...
    if (action === "checkin") {
      ...
      if (!manual && Number.isFinite(lat) && Number.isFinite(lng)) {
        ...
        if (zonesError) return bad(req, zonesError.message, 500);
