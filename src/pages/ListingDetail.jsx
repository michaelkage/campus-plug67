...
function EscrowPanel({ tx, isSeller, onRefresh }) {
  ...
  return (
    ...
    >
      ...
      {!isSeller && tx.status === 'release_requested' && <div className="space-y-3"><div className="flex items-start gap-2 p-3 rounded-xl bg-[var(--md-secondary)]/10"><AlertTriangle size={13} className="text-[var(--md-secondary)] flex-shrink-0 mt-0.5" /><p className="text-xs text-white/60">Seller requested release. Funds auto-release in <strong className="text-[var(--md-secondary)]">{hoursLeft}h {minsLeft}m</strong>. Not received the item?</p></div><button onClick={() => setShowDispute(v => !v)} className="w-full py-2.5 rounded-lg text-sm font-semibold border border-[var(--md-error)]/30 text-[var(--md-error)] hover:bg-[var(--md-error)]/10 transition-colors">🚨 File a Dispute</button><AnimatePresence>{showDispute && <motion.div initial={{ opacity:0, height:0 }} animate={{ opacity:1, height:'auto' }} exit={{ opacity:0, height:0 }} className="overflow-hidden space-y-2"><textarea className="input resize-none w-full text-sm" rows={3} placeholder="Describe the issue (min 20 chars)..." value={disputeText} onChange={e => setDisputeText(e.target.value)} /><button onClick={() => edge('dispute', { reason: disputeText })} disabled={loading === 'dispute' || disputeText.length < 20} className="btn-primary w-full text-sm disabled:opacity-40">{loading === 'dispute' ? 'Filing...' : 'Submit Dispute'}</button></motion.div>}</AnimatePresence></div>}
      {tx.status === 'released' && <motion.div initial={{ scale:0.8, opacity:0 }} animate={{ scale:1, opacity:1 }} className="text-center space-y-2 py-2"><div className="text-3xl">🎉</div><div className="font-bold text-[var(--md-primary)]">Exchange Complete!</div><p className="text-xs text-white/40">{isSeller ? 'Funds released to your account.' : 'Enjoy your item!'}</p></motion.div>}
      {tx.status === 'disputed' && <div className="text-center space-y-2"><p className="text-sm text-white/50">Dispute filed. Review within 24 hours.</p>{tx.dispute_reason && <div className="p-3 rounded-xl bg-[var(--md-surface-container-high)] text-xs text-white/40 text-left"><span className="font-semibold text-white/60">Reason: </span>{tx.dispute_reason}</div>}</div>}
    </motion.div>
  )
}
    ...
    <div className="page-shell max-w-4xl">
      ...
      <div className="grid md:grid-cols-5 gap-8">
        <div className="md:col-span-3 space-y-3">
          ...
          {images.length > 1 && <div className="flex gap-2">{images.map((img, i) => <button key={i} onClick={() => setActiveImg(i)} className={`w-16 h-16 rounded-lg overflow-hidden border-2 transition-colors ${activeImg === i ? 'border-cyan' : 'border-[var(--md-outline-variant)]'}`}>{img ? <img src={img} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-[var(--md-surface-container-high)] flex items-center justify-center">📦</div>}</button>)}</div>}
