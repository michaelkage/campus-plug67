...
DECLARE actor uuid := auth.uid(); raw_token text := encode(gen_random_bytes(32), 'hex'); hash text := encode(digest(raw_token, 'sha256'), 'hex'); handoff_id uuid;
BEGIN
  IF actor IS NULL THEN RAISE EXCEPTION 'Authentication required'; END IF;
  IF p_transaction_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM public.transactions t WHERE t.id=p_transaction_id AND (t.buyer_id=actor OR t.seller_id=actor)) THEN RAISE EXCEPTION 'Not authorized for this transaction'; END IF;
  DELETE FROM public.session_handoffs WHERE user_id=actor AND (consumed_at IS NOT NULL OR expires_at < now());
  INSERT INTO public.session_handoffs(user_id,transaction_id,token_hash,expires_at) VALUES(actor,p_transaction_id,hash,now()+interval '2 minutes') RETURNING id INTO handoff_id;
...
RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE row public.session_handoffs%ROWTYPE;
BEGIN
  IF p_token IS NULL OR length(p_token) < 32 THEN RETURN jsonb_build_object('success',false,'error','Invalid handoff token'); END IF;
  SELECT * INTO row FROM public.session_handoffs WHERE token_hash=encode(digest(p_token,'sha256'),'hex') AND consumed_at IS NULL AND expires_at > now() FOR UPDATE;
...
ALTER TABLE public.transactions ADD COLUMN IF NOT EXISTS mobile_release_ready_at timestamptz;

CREATE OR REPLACE FUNCTION public.record_safe_arrival_v2(p_transaction_id uuid,p_role text,p_lat numeric,p_lng numeric,p_user_id uuid,p_device_type text)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE tx public.transactions%ROWTYPE; uni text; z record;
BEGIN
  IF p_device_type <> 'mobile' THEN RETURN jsonb_build_object('success',false,'code','MOBILE_REQUIRED','message','Meetup release must be completed on a phone with GPS.'); END IF;
  ...
  IF p_role='buyer' THEN
    UPDATE public.transactions SET buyer_arrived=true,buyer_arrival_time=now(),buyer_lat=p_lat,buyer_lng=p_lng,buyer_location_captured_at=now(),buyer_safe_zone_id=z.zone_id,mobile_release_ready_at=now() WHERE id=tx.id;
  ELSE
...
-- Hard backstop: normal authenticated release can only happen after a fresh
-- server-gated mobile arrival. Service-role auto-release is intentionally exempt.
CREATE OR REPLACE FUNCTION public.guard_mobile_escrow_release()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
BEGIN
  IF NEW.status='released' AND OLD.status IS DISTINCT FROM 'released' AND auth.role() <> 'service_role' THEN
    IF NEW.mobile_release_ready_at IS NULL OR NEW.mobile_release_ready_at < now()-interval '10 minutes' THEN
      RAISE EXCEPTION 'Mobile meetup verification is required before QR release';
    END IF;
  END IF;
  RETURN NEW;
END; $$;
DROP TRIGGER IF EXISTS transactions_mobile_release_guard ON public.transactions;
CREATE TRIGGER transactions_mobile_release_guard BEFORE UPDATE OF status ON public.transactions FOR EACH ROW EXECUTE FUNCTION public.guard_mobile_escrow_release();
...
UPDATE public.passkey_credentials SET transports=ARRAY['internal','hybrid']::text[] WHERE transports IS NULL OR cardinality(transports)=0;
CREATE INDEX IF NOT EXISTS transactions_mobile_release_idx ON public.transactions(id,buyer_location_captured_at) WHERE status='meetup_initiated';
