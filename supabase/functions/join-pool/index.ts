...
serve(async(req:Request)=>{
  ...
  const paystackRef=typeof body.paystack_ref==="string"?body.paystack_ref:null;
  if(!poolId)return bad("Missing pool_id");

  const {data,error:resultError}=await admin.rpc("atomic_pool_join",{p_pool_id:poolId,p_user_id:user.id,p_ref:paystackRef});
  if(resultError)return bad(resultError.message,500);
  if(!data?.success){
    const status=data?.reason==="pool_not_found"?404:400;
    return bad(data?.reason??"Unable to join pool",status);
  }
