...
export function CampusTicker({ university }) {
  ...
  useEffect(() => {
    ...
    supabase.from('ticker_events').select('*').eq('university', university).gt('expires_at', new Date().toISOString()).order('created_at', { ascending: false }).limit(20).then(({ data }) => {
      if (data?.length) { queueRef.current = [...data].reverse(); rotate() }
    })
    const channel = supabase.channel(`ticker:${university}`).on('postgres_changes', {
      event: 'INSERT', schema: 'public', table: 'ticker_events', filter: `university=eq.${university}`,
    }, (payload) => { queueRef.current = [...queueRef.current, payload.new] }).subscribe()
    return () => {
  ...
  const rotate = () => {
    if (!queueRef.current.length) { timerRef.current = setTimeout(rotate, 8000); return }
    const [next, ...rest] = queueRef.current
    queueRef.current = rest.length ? [...rest, next] : [next]
    setCurrent(next)
    timerRef.current = setTimeout(() => { setCurrent(null); timerRef.current = setTimeout(rotate, 800) }, 4500)
  }
...
export function DemandBroadcastForm() {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      const { data, error } = await supabase.from('buyer_broadcast_demands').insert({ buyer_id: user?.id, title, max_budget: parseInt(budget, 10) * 100, category_id: parseInt(categoryId, 10) }).select()
      if (error) throw error
      return data
    },
    onSuccess: () => { toast.success('Demand broadcasted to campus! Sellers will be notified.'); setTitle(''); setBudget('') },
...
export function DemandStream() {
  ...
  useEffect(() => {
    supabase.from('buyer_broadcast_demands').select('*, profiles(full_name)').order('created_at', { ascending: false }).limit(5).then(({ data }) => { if (data) setDemands(data) })
    const channel = supabase.channel('public:buyer_broadcast_demands').on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'buyer_broadcast_demands' }, (payload) => {
