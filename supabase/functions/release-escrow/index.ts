...
serve(async(req:Request)=>{
  ...
  const transactionId=typeof record.transaction_id==='string'?record.transaction_id:"";if(!transactionId)return jsonResponse({error:"Missing transaction_id"},400,{},req);
  ...
  if(error){const message=error.message||"Escrow action failed";const status=/not authorized|only the|invalid release credential|authentication/i.test(message)?403:400;return jsonResponse({error:message},status,{},req);}
