-- Prevent multiple live marketplace purchases for the same listing.
CREATE UNIQUE INDEX IF NOT EXISTS transactions_one_active_per_listing
  ON public.transactions(listing_id)
  WHERE status IN ('pending','locked','meetup_initiated','release_requested','disputed');

-- Keep paid listings out of the active marketplace once escrow is locked.
...
BEGIN
  ...
  UPDATE public.listings
    ...
    WHERE id=tx.listing_id AND status='active';
