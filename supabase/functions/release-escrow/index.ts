...
serve(async(req:Request)=>{
  ...
  const token=getBearerToken(req);if(!token)return jsonResponse({error:"Unauthorized"},401,{},req);
  try{const limit=await enforceRateLimitWithToken(token,"escrow-actions",30,60);if(!limit.allowed)return jsonResponse({error:"Rate limit exceeded"},429,{},req);}catch{return jsonResponse({error:"Rate limit service unavailable"},503,{},req);}
  ...
  // Duress is a separate safety state transition. It never enters the normal QR release path.
  if(action==="duress"){
    ...
    if(error){const message=error.message||"Duress activation failed";const status=/not authorized|invalid duress|authentication/i.test(message)?403:400;return jsonResponse({error:message},status,{},req);}
    return jsonResponse(data,200,{},req);
  }

  const rpcArgs={p_transaction_id:transactionId,p_action:action,p_qr_secret:typeof record.qr_secret==='string'?record.qr_secret:(typeof record.release_code==='string'?record.release_code:null),p_reason:typeof record.reason==='string'?record.reason:null};
  let userClient;try{userClient=createUserClient(token);}catch{return jsonResponse({error:"Auth client unavailable"},503,{},req);}
  const {data,error}=await userClient.rpc("process_escrow_action",rpcArgs);
  if(error){const message=error.message||"Escrow action failed";const status=/not authorized|only the|invalid release credential|frozen by a safety alert|authentication/i.test(message)?403:400;return jsonResponse({error:message},status,{},req);}
  return jsonResponse(data,200,{},req);
});
